...
// EventBridge content filters only support `prefix`/`suffix`/`anything-but` per
// field — no regex. The rule fires on every Object Created under `users/`,
// including thumbnail writes (`users/*/thumbnails/*`). The real thumbnail-generator
// (story 4-5) MUST short-circuit on keys that do not match `*/originals/*` to avoid
// an invocation loop. The current stub handler is safe because it no-ops.
export class LambdaStack extends Stack {
  ...
  public readonly orphanReconcilerDlq: Queue;
  ...
  constructor(scope: Construct, id: string, props: LambdaStackProps) {
    ...
    this.orphanReconcilerDlq = new Queue(this, 'OrphanReconcilerDlq', {
      queueName: `cloudvault-${props.envName}-orphan-reconciler-dlq`,
      retentionPeriod: Duration.days(14),
      encryption: QueueEncryption.SQS_MANAGED,
    });
    ...
    props.storageBucket.grantRead(this.orphanReconciler, 'users/*');
    ...
    new Rule(this, 'OrphanReconcilerSchedule', {
      ...
      targets: [
        new LambdaFunction(this.orphanReconciler, {
          deadLetterQueue: this.orphanReconcilerDlq,
          retryAttempts: 2,
        }),
      ],
    });
