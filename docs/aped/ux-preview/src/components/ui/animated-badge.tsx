...
export const AnimatedBadge = ({
  ...
  const content = (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group relative flex max-w-fit items-center justify-center gap-3 rounded-full border border-[var(--border)] bg-[var(--background)] px-4 py-1.5 text-[var(--foreground)] transition-colors"
    >
      ...
      <ChevronRight className="ml-1 h-3.5 w-3.5 text-[var(--muted-foreground)] transition-transform duration-200 group-hover:translate-x-0.5" />
    </motion.div>
  );
